...
class XmlAccessor {
  ...
  get weekdayItemMap() {
    return chain(this.weekdayItems)
      .groupBy(
        (item) =>
          castArray(item["wp:postmeta"]).find((meta) => meta["wp:meta_key"]["$"] === "weekday")[
            "wp:meta_value"
          ]["$"],
      )
...
class XmlToCsv {
  ...
  static colors = Object.freeze([
    ...Object.values(materialColors)
      .filter((color) => typeof colors === "object" && color.hasOwnProperty("50"))
      .map((color) => color["50"]),
    ...Object.values(materialColors)
      .filter((color) => typeof colors === "object" && color.hasOwnProperty("300"))
      .map((color) => color["300"]),
  ]);
}
...
export async function csvToWordPressTimetable({ csvFile, xmlFile }) {
  ...
  return create({ version: "1.0", encoding: "UTF-8" }).ele(newXml).end({ prettyPrint: true });
}

async function parseCsv(csvFile) {
  console.info("loading CSV");
  const csv = [];
  const parser = fs.createReadStream(csvFile).pipe(csvParser());
  parser.on("readable", () => {
    let data;
